...
$id_vaksin = $koneksi->real_escape_string($_GET['id_vaksin']);
$a = $koneksi -> query("SELECT * FROM vaksin WHERE id_vaksin = '$id_vaksin'");
$data = $a->fetch_assoc();
?>
<!DOCTYPE html>
<html>
...
if (isset($_POST['submit'])) {
  ...
  $vaksin = $_POST['vaksin'];
  try {
    $a = $koneksi->query("UPDATE `vaksin` SET `nama`='$nama',`nik`='$nik',`usia`='$usia',`vaksin`='$vaksin' WHERE `id_vaksin`='$id_vaksin'");
    ?>
    <script>
      Swal.fire({
        icon: 'success',
