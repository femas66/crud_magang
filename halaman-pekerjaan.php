...
<div class="w3-container">
  <br>
  <a href="pekerjaan.php" class="tambah"><i class="fa-solid fa-plus"></i> Tambah</a>
  <hr>
  <table class="tabel">
    <tr class="judulatas">
      <th>#</th>
    ...
    <?php
    $pekerjaan = $koneksi -> query("SELECT * FROM pekerjaan");
    if ($pekerjaan -> num_rows > 0) {
      ...
      while ($row = $pekerjaan ->fetch_assoc()) {
        ...
        <tr>
          ...
          <td><?= $row['alamat'] ?></td>
          <td><?= $row['gaji'] ?></td>
          <td><a href="edit-pekerjaan.php?id_pekerjaan=<?= $row['id_pekerjaan'] ?>" style="color: black;"><i class="fa-solid fa-pen-to-square" style="color: black;"></i> Edit</a></td>
          <td><a href="?hapus_pekerjaan=<?= $row['id_pekerjaan'] ?>"><i class="fa-solid fa-trash" style="color: red;"></i> Hapus</a></td>
        </tr>
        <?php
      }
    }
...
if (isset($_GET['hapus_pekerjaan'])) {
    ...
    Swal.fire({
      ...
      if (result.isConfirmed) {
        window.location = "hapus-data.php?hapus_pekerjaan=<?= $_GET['hapus_pekerjaan'] ?>";
      }
    })
    </script>
