...
$id_agama = $koneksi->real_escape_string($_GET['id_agama']);
$a = $koneksi -> query("SELECT * FROM agama WHERE id_agama = '$id_agama'");
$data = $a->fetch_assoc();
?>
<!DOCTYPE html>
<html>
...
if (isset($_POST['submit'])) {
  ...
  include 'koneksi.php';
  $a = $koneksi->query("UPDATE `agama` SET `nama`='$nama',`jenis_kelamin`='$jk',`agama`='$agama',`agama_sebelumnya`='$agama_sebelumnya' WHERE id_agama = '$id_agama'");
  ?>
  <script>
    Swal.fire({
        icon: 'success',
