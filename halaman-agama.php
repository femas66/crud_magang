...
<div class="w3-container">
  <br>
  <a href="agama.php" class="tambah"><i class="fa-solid fa-plus"></i> Tambah</a>
  <hr>
  <table class="tabel">
    <tr class="judulatas">
      <th>#</th>
    ...
    <?php
    $agama = $koneksi -> query("SELECT * FROM agama");
    if ($agama -> num_rows > 0) {
      $i = 1;
      while ($row = $agama ->fetch_assoc()) {
        ?>
        <tr>
          <td><?= $i++ ?></td>
          <td><?= $row['nama'] ?></td>
          ...
          <td><?= $row['agama'] ?></td>
          <td><?= $row['agama_sebelumnya'] ?></td>
          <td><a href="edit-agama.php?id_agama=<?= $row['id_agama'] ?>"><i class="fa-solid fa-pen-to-square" style="color: black;"></i> Edit</a></td>
          <td><a href="?hapus_agama=<?= $row['id_agama'] ?>"><i class="fa-solid fa-trash" style="color: red;"></i> Hapus</a></td>
        </tr>
        <?php
      }
    }
    ...
    <script>
      Swal.fire({
        ...
        if (result.isConfirmed) {
          window.location = "hapus-data.php?hapus_agama=<?= $_GET['hapus_agama'] ?>";
        }
      })
    </script>
    <?php
