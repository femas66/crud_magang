...
<div class="w3-container" style="background: #45ADA8; color: white">
  <h1>Halaman Vaksin</h1>
</div>

<div class="w3-container">
  <br>
  <a href="vaksin.php" class="tambah"><i class="fa-solid fa-plus"></i> Tambah</a>
  <hr>
  <table class="tabel">
    <tr class="judulatas">
      <th>#</th>
    ...
    <?php
    $vaksin = $koneksi -> query("SELECT * FROM vaksin");
    if ($vaksin -> num_rows > 0) {
      $i = 1;
      while ($row = $vaksin ->fetch_assoc()) {
        ?>
        <tr>
          <td><?= $i++ ?></td>
          <td><?= $row['nama'] ?></td>
          <td><?= $row['nik'] ?></td>
          <td><?= $row['usia'] ?></td>
          <td><?= ($row['vaksin'] == 'Y') ? "Sudah" : "Belum" ?></td>
          <td><a href="edit-vaksin.php?id_vaksin=<?= $row['id_vaksin'] ?>"><i class="fa-solid fa-pen-to-square" style="color: black;"></i> Edit</a></td>
          <td><a href="?hapus_vaksin=<?= $row['id_vaksin'] ?>"><i class="fa-solid fa-trash" style="color: red;"></i> Hapus</a></td>
        </tr>
        <?php
      }
    }
    ...
    <script>
      Swal.fire({
        ...
        if (result.isConfirmed) {
          window.location = "hapus-data.php?hapus_vaksin=<?= $_GET['hapus_vaksin'] ?>";
        }
      })
